...
namespace UnityEditor
{
    ...
    /// </summary>
    [CustomEditor(typeof(GlobalRuleTile), true)]
    [CanEditMultipleObjects]
    public class GlobalRuleTileEditor : Editor
    {
        ...
        /// </summary>
        public GlobalRuleTile tile => target as GlobalRuleTile;
        /// <summary>
        ...
        public virtual BoundsInt GetRuleGUIBounds(BoundsInt bounds, GlobalRuleTile.TilingRule rule)
        {
            if (extendNeighbor)
            {
                bounds.xMin--;
        ...
        {
            GlobalRuleTile.TilingRule rule = tile.m_TilingRules[index];
        ...
        public float GetElementHeight(GlobalRuleTile.TilingRule rule)
        {
            BoundsInt bounds = GetRuleGUIBounds(rule.GetBounds(), rule);

            float inspectorHeight = GetElementHeight(rule as GlobalRuleTile.TilingRuleOutput);
        ...
        private void OnAddElement(object obj)
        {
            ...
            GlobalRuleTile.TilingRule rule = new GlobalRuleTile.TilingRule();
            rule.m_Output = GlobalRuleTile.TilingRule.OutputSprite.Single;
        ...
        public static void UpdateAffectedOverrideTiles(GlobalRuleTile target)
        {
        ...
        public static List<RuleOverrideTile> FindAffectedOverrideTiles(GlobalRuleTile target)
        {
        ...
        public virtual void RuleOnGUI(Rect rect, Vector3Int position, int neighbor)
        {
            switch (neighbor)
            {
                case GlobalRuleTile.TilingRule.Neighbor.This:
                    GUI.DrawTexture(rect, arrows[GetArrowIndex(position)]);
                    break;
                case GlobalRuleTile.TilingRule.Neighbor.NotThis:
        ...
        public virtual void RuleTransformOnGUI(Rect rect, GlobalRuleTile.TilingRule.Transform ruleTransform)
        {
            switch (ruleTransform)
            {
                case GlobalRuleTile.TilingRule.Transform.Rotated:
                    GUI.DrawTexture(rect, autoTransforms[0]);
                    break;
                case GlobalRuleTile.TilingRule.Transform.MirrorX:
                    GUI.DrawTexture(rect, autoTransforms[1]);
                    break;
                case GlobalRuleTile.TilingRule.Transform.MirrorY:
                    GUI.DrawTexture(rect, autoTransforms[2]);
                    break;
                case GlobalRuleTile.TilingRule.Transform.Fixed:
                    GUI.DrawTexture(rect, autoTransforms[3]);
                    break;
                case GlobalRuleTile.TilingRule.Transform.MirrorXY:
        ...
        public void RuleNeighborUpdate(Rect rect, GlobalRuleTile.TilingRule tilingRule, Dictionary<Vector3Int, int> neighbors, Vector3Int position)
        {
        ...
        public void RuleTransformUpdate(Rect rect, GlobalRuleTile.TilingRule tilingRule)
        {
            if (Event.current.type == EventType.MouseDown && ContainsMousePosition(rect))
            {
                tilingRule.m_RuleTransform = (GlobalRuleTile.TilingRule.Transform)(int)Mathf.Repeat((int)tilingRule.m_RuleTransform + GetMouseChange(), Enum.GetValues(typeof(GlobalRuleTile.TilingRule.Transform)).Length);
                GUI.changed = true;
                Event.current.Use();
            }
        }
        ...
        public virtual void RuleMatrixOnGUI(GlobalRuleTile tile, Rect rect, BoundsInt bounds, GlobalRuleTile.TilingRule tilingRule)
        {
        ...
        public void RuleMatrixIconOnGUI(GlobalRuleTile.TilingRule tilingRule, Dictionary<Vector3Int, int> neighbors, Vector3Int position, Rect rect)
        {
        ...
        public virtual void SpriteOnGUI(Rect rect, GlobalRuleTile.TilingRuleOutput tilingRule)
        {
        ...
        public void RuleInspectorOnGUI(Rect rect, GlobalRuleTile.TilingRuleOutput tilingRule)
        {
            float y = rect.yMin;
            ...
            tilingRule.m_Output = (GlobalRuleTile.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
            y += k_SingleLineHeight;

            if (tilingRule.m_Output == GlobalRuleTile.TilingRule.OutputSprite.Animation)
            {
            ...
            if (tilingRule.m_Output == GlobalRuleTile.TilingRule.OutputSprite.Random)
            {
                ...
                tilingRule.m_RandomTransform = (GlobalRuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
                y += k_SingleLineHeight;
            }

            if (tilingRule.m_Output != GlobalRuleTile.TilingRule.OutputSprite.Single)
            {
        ...
        class RuleTileRuleWrapper
        {
            ...
            /// </summary>
            [SerializeField]
            public List<GlobalRuleTile.TilingRule> rules = new List<GlobalRuleTile.TilingRule>();
        }
        ...
        [MenuItem("CONTEXT/GlobalRuleTile/Copy All Rules")]
        public static void CopyAllRules(MenuCommand item)
        {
            GlobalRuleTile tile = item.context as GlobalRuleTile;
            if (tile == null)
                return;
        ...
        /// <summary>
        /// Pastes all Rules from the clipboard to a GlobalRuleTile
        /// </summary>
        /// <param name="item">MenuCommand storing the GlobalRuleTile to paste to</param>
        [MenuItem("CONTEXT/GlobalRuleTile/Paste Rules")]
