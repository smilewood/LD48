...
namespace UnityEngine
{
    ...
    [CreateAssetMenu(fileName ="GlobalRuleTle", menuName ="Tiles/GlobalRuleTile")]
    public class GlobalRuleTile : TileBase
    {
        /// <summary>
        ...
        /// </summary>
        [HideInInspector] public List<TilingRule> m_TilingRules = new List<GlobalRuleTile.TilingRule>();
        ...
        static void CachingTilemapNeighborPositions(Tilemap tilemap)
        {
            ...
            for (int i = 0; i < usedTileCount; i++)
            {
                ...
                usedTiles.Add(tile);
                GlobalRuleTile ruleTile = null;

                if (tile is GlobalRuleTile)
                    ruleTile = tile as GlobalRuleTile;
        ...
        public override void RefreshTile(Vector3Int position, ITilemap tilemap)
        {
            ...
            foreach (Vector3Int offset in neighborPositions)
            {
                ...
                //TileBase tile = GetTileInNeighborBone(tilemap, position, offsetPosition);
        ...
        public FieldInfo[] GetCustomFields(bool isOverrideInstance)
        {
            return this.GetType().GetFields()
                .Where(field => typeof(GlobalRuleTile).GetField(field.Name) == null)
                .Where(field => !field.IsDefined(typeof(HideInInspector)))
                .Where(field => !isOverrideInstance || !field.IsDefined(typeof(GlobalRuleTile.DontOverride)))
                .ToArray();
        }
        ...
        public bool RuleMatches(TilingRule rule, Vector3Int position, ITilemap tilemap, int angle)
        {
            for (int i = 0; i < rule.m_Neighbors.Count && i < rule.m_NeighborPositions.Count; i++)
            {
                ...
                TileBase other = GetTileInNeighborBone(tilemap, position, positionOffset);
        ...
        private TileBase GetTileInNeighborBone(ITilemap origin, Vector3Int position, Vector3Int positionOffset)
        {
            if (Application.isPlaying)
            {
                Tilemap baseTilemap = origin.GetComponent<Tilemap>();
                Vector2 bonepos = baseTilemap.gameObject.transform.parent.GetComponent<BoneItializer>().position;
                
                Bone[] neighbors = GetNeighboringBones(bonepos);
                Bone thisBone = GetBoneAtPos(bonepos);
                Vector3Int worldPos = GetOffsetPosition(position, positionOffset) + new Vector3Int((int)(bonepos.x * 32), (int)(bonepos.y * 32), 0);
                

                //Debug.Log(string.Format("Search for neighbor {0} of tile {1}; not found in bone {2} WorldPos: {3}", positionOffset, position, thisBone.GameObject.name, worldPos));

                if (!thisBone.PointInBone((Vector3)worldPos))
                {
                    //Debug.Log("Not in thisBone");
                    //find correct neighbor
                    foreach (Bone b in neighbors)
                    {
                        if (b.PointInBone((Vector3)worldPos))
                        {
                            Vector3Int worldToBone = worldPos - new Vector3Int((int)(b.origin.x * 32), (int)(b.origin.y * 32), 0);
                            //Debug.Log(string.Format("neighbor {0} of tile {1} could be in bone {2} at {3}", positionOffset, position, b.GameObject.name, worldToBone));
                            return b.floorTilemap.GetTile(worldToBone);
                        }
                        else
                        {
                            //Debug.Log("not in neighbor " + b.GameObject.name);
                        }
                    }
                }
            }
            return origin.GetTile(GetOffsetPosition(position, positionOffset));
        }
